...
def gluon_to_mxnet(name):
    ...
    # downsample to sc
    name = name.replace("downsample_1", "sc")
    name = name.replace("downsample_2", "sc_bn")
    # running to moving
    name = name.replace("running", "moving")
    # conv1 to conv0
    if name.startswith("conv1_0"):
        name = name.replace("conv1_0", "conv0_0")
    if name.startswith("conv1_1"):
        name = name.replace("conv1_1", "bn0_0")
    if name.startswith("conv1_3"):
        name = name.replace("conv1_3", "conv0_1")
    if name.startswith("conv1_4"):
        name = name.replace("conv1_4", "bn0_1")
    if name.startswith("conv1_6"):
        name = name.replace("conv1_6", "conv0_2")
    if name.startswith("bn1"):
        name = name.replace("bn1", "bn0_2")
    # add prefix
    if "moving" in name:
        return "aux:" + name
    else:
